...
import { CertificateTemplatesManager } from './CertificateTemplatesManager';
import {
  getSavedFonts,
  saveSavedFonts,
  resetSavedFonts,
...
interface DirectoryInspectorProps {
  onSelectTemplateForIssuer?: (template: CertificateTemplate) => void;
  initialSubTab?: 'templates' | 'identity';
}

export const DirectoryInspector: React.FC<DirectoryInspectorProps> = ({
  onSelectTemplateForIssuer,
  initialSubTab = 'templates',
}) => {
  const [settingsTab, setSettingsTab] = useState<'templates' | 'identity'>(initialSubTab);
  ...
  return (
    <div className="flex flex-col gap-8 max-w-6xl mx-auto pb-12">
      ...
      {/* NAVEGAÇÃO ENTRE AS SUB-ABAS DE CONFIGURAÇÃO */}
      <div className="flex flex-wrap items-center gap-3 border-b-2 border-slate-200 pb-2">
        <button
          type="button"
          onClick={() => setSettingsTab('templates')}
          className={`flex items-center gap-2.5 px-5 py-3 rounded-xl font-bold text-xs sm:text-sm transition-all ${
            settingsTab === 'templates'
              ? 'bg-[#0F2C59] text-amber-300 shadow-lg ring-2 ring-amber-400 font-extrabold'
              : 'bg-white text-slate-700 hover:bg-slate-100 border border-slate-200 shadow-sm'
          }`}
        >
          <Layers className="w-4 h-4 text-amber-400" />
          <span>Modelos de Certificados (PDF)</span>
          <span className="bg-amber-400 text-[#0F2C59] text-[10px] font-black px-2 py-0.5 rounded-full uppercase shadow-xs">
            Core
          </span>
        </button>
        ...
        >
          ...
          <span>Identidade Visual & Tipografia</span>
        </button>
      </div>

      {/* CONTEÚDO DA ABA SELECIONADA */}
      {settingsTab === 'templates' ? (
        <CertificateTemplatesManager onSelectTemplateForIssuer={onSelectTemplateForIssuer} />
      ) : (
        /* PAINEL PRINCIPAL EM DUAS COLUNAS (LOGO E FONTES) */
